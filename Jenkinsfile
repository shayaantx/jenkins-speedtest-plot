...
pipeline {
        ...
        stage('write results') {
            steps {
                script {
                    ...
                    String graphData = "download,upload\n";
                    int i = 0;
    ...
    post {
        always {
            plot csvFileName: 'plot-856fbd0f-69e7-44df-9867-8d8f598dabeb.csv', csvSeries: [[displayTableFlag: true, exclusionValues: 'downloads,uploads', file: 'data.csv', inclusionFlag: 'OFF', url: '']], group: 'speedtest', style: 'line', title: 'speedtest', yaxis: 'mbps'
            deleteDir()
        }
    }
}